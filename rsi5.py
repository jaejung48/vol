...
# 반복문 실행
    while True:  
        ...
        #########################################################################
            ...
            #########################################################################
            lp_ohlcv = pyupbit.get_ohlcv(ticker, "minute5", 20)
            lp = lp_ohlcv['low']
            last_lp = lp[19]
            min_lp = round(lp.min(axis = 0))
        ...
        # print('전환선: ',ten.iloc[-1])
            ...
            if rsi1 < rsi and ten.iloc[-1] < current_price < bu20_m5_199 :
                print(ticker, "첫번째")
                if ten.iloc[-1] > kijun.iloc[-1] and ma5_m5_trend_199 > 0 :
                    if current_price > senkoua.iloc[-1] and current_price > senkoub.iloc[-1] :
                        if current_price >= m5_max_high_180_199  and bu20_m5_198 > m5_max_high_180_198 :
            ...
            if abp == 0 :
                if previous_min_lp > min_lp :
                    nlt = 1
                    previous_min_lp = min_lp
